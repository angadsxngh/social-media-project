...
//   console.log(userid)
  ...
  useEffect(() => {
    async function fetchUser() {
      try {
        ...
        // const postResponse = await fetch(`/api/v1/users/profile/${params.userid}`,
        //     {
        //         method: "GET",
        //     }
        // )
  ...
  if (error) return <div className="text-red-500 text-center mt-10">{error.message}</div>;
    ...
    <div className="bg-black min-h-screen flex flex-col items-center p-10 text-white">
      <div className="bg-[#111] p-6 rounded-lg shadow-lg w-full max-w-md text-center">
        ...
        <div className="mt-5 flex justify-around text-sm text-gray-300">
          <div>
            <p className="font-semibold">{user?user.posts.length:"no user"}</p>
            <p>Posts</p>
          </div>
          <div>
            <p className="font-semibold">{user?user.followers.length:"no user"}</p>
            <p>Followers</p>
          </div>
          <div>
            <p className="font-semibold">{user?user.following.length:"no user"}</p>
            <p>Following</p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 w-full max-w-md">
        ...
        {user.posts.length > 0 ? (
          <ul className="space-y-4">
            {user.posts.map((post) => (
              <li key={post.id} className="bg-[#222] p-3 rounded-lg shadow-md">
                <p>{post.caption}</p>
              </li>
            ))}
          </ul>
